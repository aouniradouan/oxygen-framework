...
class View
{
    ...
    protected function addGlobals()
    {
        ...
        // Auth Functions
        $this->twig->addFunction(new TwigFunction('auth_check', function () use ($auth) {
            return $auth->check();
        }));

        $this->twig->addFunction(new TwigFunction('auth_user', function () use ($auth) {
            return $auth->user();
        }));
        ...
        $this->twig->addFunction(new TwigFunction('locale', function () {
            return Lang::getLocale();
        }));

        // JSON Decode
        $this->twig->addFunction(new TwigFunction('json_decode', function ($json, $assoc = true) {
